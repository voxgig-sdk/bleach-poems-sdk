...
class BleachPoemsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BleachPoemsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BleachPoemsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BleachPoemsTestRunner.getenv("BLEACH_POEMS_TEST_LIVE")
        override = BleachPoemsTestRunner.getenv("BLEACH_POEMS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BleachPoemsTestRunner.getenv(key)
        ...
        explain = BleachPoemsTestRunner.getenv("BLEACH_POEMS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BleachPoemsTestRunner._test_control is not None:
            return BleachPoemsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BleachPoemsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BleachPoemsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BleachPoemsTestRunner.load_test_control()
...
def load_env_local():
    BleachPoemsTestRunner.load_env_local()


def env_override(m):
    return BleachPoemsTestRunner.env_override(m)


def entity_data(v):
    return BleachPoemsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BleachPoemsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BleachPoemsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BleachPoemsTestRunner.load_test_control()


def live_delay_ms():
    return BleachPoemsTestRunner.live_delay_ms()
