...
// BleachPoems SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        BleachPoemsSDK $client,
        BleachPoemsUtility $utility
    ): BleachPoemsContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new BleachPoemsContext($ctxmap, null);
    ...
    private static function fixctx(BleachPoemsContext $ctx, BleachPoemsSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?BleachPoemsError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BleachPoemsError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new BleachPoemsResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new BleachPoemsBaseFeature();
    ...
    {
        $hook_client = BleachPoemsSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends BleachPoemsBaseFeature {
            public $hook_fn = null;
            public function TestHook(BleachPoemsContext $ctx): void
            {
    ...
    {
        $init_client = BleachPoemsSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends BleachPoemsBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new BleachPoemsSDK([
            'system' => [
    ...
    {
        $blocked_client = new BleachPoemsSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new BleachPoemsSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = BleachPoemsSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new BleachPoemsOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = BleachPoemsSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof BleachPoemsError) ? $result->err->msg : (string)$result->err;
