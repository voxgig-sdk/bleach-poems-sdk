...
class PoemEntity
  ...
  def make
    opts = @_entopts.dup
    PoemEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BleachPoemsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Poem, Hash] the current Poem data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BleachPoemsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Poem fields)
  def match_get
  ...
  # Load a single Poem.
  #
  # @param reqmatch [PoemLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Poem.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Poem, Hash] the loaded Poem; raises BleachPoemsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BleachPoemsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PoemListMatch, Hash, nil] match filter (any subset of
  #   Poem fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Poem>, Array] the matching Poem items; raises BleachPoemsError on failure
