...
module BleachPoemsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BleachPoemsResponse.new({})
    ...
    if fetch_err
      ...
      response = BleachPoemsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BleachPoemsResponse.new(fetched)
    else
