...
boru-driven command-line client **and** interactive REPL for the BleachPoems
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bleach-poems-cli)
make build

# 2. See usage (words, entities, env vars)
./bleach-poems-cli --help
...
./bleach-poems-cli load 1 poem            # {id:1} shorthand
./bleach-poems-cli load '{id:1}' poem       # explicit match map

# 5. Override the API base URL for a single call
BLEACH_POEMS_BASE=https://api.example.com ./bleach-poems-cli list poem

# 6. No arguments -> interactive REPL
./bleach-poems-cli
bleach-poems> list poem
bleach-poems> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bleach-poems-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bleach-poems-cli list poem
   ```
...
```sh
./bleach-poems-cli load 1 poem          # scalar shorthand for {id:1}
./bleach-poems-cli load '{id:1}' poem     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bleach-poems>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `poem`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bleach-poems-cli`. |
...
(`./bleach-poems-cli <expr>`) and interactively (the REPL), and expressions compose the
