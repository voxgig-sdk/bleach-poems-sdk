...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.BLEACHPOEMS_TEST_LIVE ||
    'TRUE' === process.env.BLEACHPOEMS_TEST_OVERRIDE
  ) {
  ...
  m.BLEACHPOEMS_TEST_EXPLAIN = process.env.BLEACHPOEMS_TEST_EXPLAIN || m.BLEACHPOEMS_TEST_EXPLAIN
