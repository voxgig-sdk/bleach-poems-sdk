...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.BLEACH_POEMS_TEST_LIVE ||
    'TRUE' === process.env.BLEACH_POEMS_TEST_OVERRIDE
  ) {
  ...
  m.BLEACH_POEMS_TEST_EXPLAIN = process.env.BLEACH_POEMS_TEST_EXPLAIN || m.BLEACH_POEMS_TEST_EXPLAIN
