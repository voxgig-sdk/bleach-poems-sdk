...
//     (\`new BleachPoemsSDK(...)\` / \`BleachPoemsSDK.test(...)\` become
//     \`BleachPoemsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"poem":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BleachPoemsSDK(...)\` and any
// \`BleachPoemsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BleachPoemsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BleachPoemsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
