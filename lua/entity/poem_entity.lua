...
local PoemEntity = {}
PoemEntity.__index = PoemEntity


function PoemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PoemEntity)
  self._name = "poem"
  self._client = client
...
function PoemEntity:get_name()
  return self._name
end


function PoemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PoemEntity.new(self._client, opts)
end
...
function PoemEntity:mark_deleted()
  self._deleted = true
end


function PoemEntity:deleted()
  return true == self._deleted
end


function PoemEntity:data_set(args)
  if args ~= nil then
...
function PoemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PoemEntity:match_set(args)
  if args ~= nil then
...
function PoemEntity:stream(action, args, callopts)
...
function PoemEntity:_run_op(ctx, post_done)
  local utility = self._utility
