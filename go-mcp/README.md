...
[MCP](https://modelcontextprotocol.io) server exposing the BleachPoems SDK as
two agent tools — `bleach-poems_list` and `bleach-poems_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bleach-poems-mcp)
make build

# 2. Provide credentials via the environment
export BLEACH_POEMS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bleach-poems \
  -- /absolute/path/to/bleach-poems-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bleach-poems-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bleach-poems_list: first page of records
{ "entity": "poem" }
{ "entity": "poem", "query": { } }

// bleach-poems_load: one record by id
{ "entity": "poem", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bleach-poems-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bleach-poems \
     -- "$PWD"/dist/*/bleach-poems-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bleach-poems_list` and `bleach-poems_load` tools now appear
   in new sessions. Ask the agent to *"list poem using bleach-poems"*
   and it calls `bleach-poems_list` with `{"entity":"poem"}`.
...
```sh
export BLEACH_POEMS_APIKEY=sk_live_xxx            # API key
export BLEACH_POEMS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `bleach-poems_list` tool
...
### Call the `bleach-poems_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bleach-poems_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bleach-poems_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BLEACH_POEMS_APIKEY` | API key sent with every request. |
| `BLEACH_POEMS_BASE` | Optional override of the API base URL. |
...
```sh
./bleach-poems-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bleach-poems_load","arguments":{"entity":"poem","query":{"id":1}}}}'
```
